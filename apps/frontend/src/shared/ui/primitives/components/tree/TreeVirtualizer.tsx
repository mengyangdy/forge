...
export interface TreeVirtualizerProps<T extends TreeItemData = TreeItemData>
  extends
    ...
    Pick<
      TreeItemProps,
      "disabledSelect" | "disabledToggle" | "indentSize" | "onSelect" | "onToggle"
    > {
  /**
   ...
   */
  renderItem?: (
    props: TreeItemRenderProps & { item: FlattenedItem<T> } & TreeRootRenderProps<T>,
  ) => ReactNode;
  /**
    ...
    (value: string) => {
      ...
      if (bubbleSelect && multiple && Array.isArray(newValue) && item) {
        const bubbleUp = (currentItem: FlattenedItem<TreeItemData>) => {
          ...
          const isAllSelected = parentItem.data.children?.every((child) =>
            (newValue as string[]).includes(child.value),
          );
          if (isAllSelected) {
