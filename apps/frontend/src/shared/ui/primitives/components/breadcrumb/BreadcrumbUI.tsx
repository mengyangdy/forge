...
const Ellipsis = <T extends BreadcrumbItem>(props: EllipsisProps<T>) => {
  ...
  return (
    <DropdownMenu items={dropdownItems} modal={false} {...ellipsisDropdownProps}>
      <BreadcrumbEllipsis className={className} {...ellipsisProps}>
        {ellipsisIcon}
      </BreadcrumbEllipsis>
    </DropdownMenu>
  );
...
const Breadcrumb = <T extends BreadcrumbItem>(props: BreadcrumbProps<T>, ref: Ref<HTMLElement>) => {
  ...
  const ellipsisItems = computedEllipsisRange
    ? items.slice(computedEllipsisRange[0], computedEllipsisRange[1])
    : [];
  ...
  return (
    <BreadcrumbRoot className={className || classNames?.root} size={size} {...rest} ref={ref}>
      <BreadcrumbList className={classNames?.list} size={size} {...listProps}>
            ...
            <Fragment key={item.value}>
              {isEllipsis ? (
                <>
                  ...
                  {separator || (
                    <BreadcrumbSeparator className={classNames?.separator} {...separatorProps} />
                  )}
                </>
              ) : null}
