...
const LayoutSidebar = ({
  ...
  const { sidebar, sidebarGapHandler, sidebarRoot, sidebarWrapper } = layoutVariants({
    collapsible,
    side,
    variant,
  });
  ...
  return isMobile ? (
    <LayoutMobile className={className} rootClassName={ui?.mobileRoot} side={side}>
      {renderChildren}
    </LayoutMobile>
  ) : (
    <div className={mergedCls.root}>
      ...
      <div className={mergedCls.wrapper}>
        <div className={mergedCls.cls} data-sidebar="sidebar">
          {renderChildren}
        </div>
      </div>
    </div>
