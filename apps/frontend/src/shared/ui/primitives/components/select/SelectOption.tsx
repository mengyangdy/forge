...
const SelectOption = (props: SelectOptionProps) => {
  ...
  if (isSeparator(item)) {
    return <SelectSeparator className={classNames?.separator} size={size} {...item} />;
  }
  ...
  if (isGroup(item)) {
    ...
    return (
      <Group className={cn(classNames?.group)} data-slot="select-group">
        <SelectLabel {...rest} className={classNames?.groupLabel}>
          {label}
        </SelectLabel>
  ...
  return (
    <SelectItem classNames={classNames} indicatorIcon={indicatorIcon} size={size} {...rest}>
      {label}
    </SelectItem>
  );
};
