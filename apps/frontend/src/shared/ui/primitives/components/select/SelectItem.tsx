...
const SelectItem = forwardRef<ComponentRef<typeof Item>, SelectItemProps>((props, ref) => {
  const { children, className, classNames, indicatorIcon, leading, size, trailing, ...rest } =
    props;
  ...
  return (
    <Item {...rest} className={mergedCls.itemCls} data-slot="select-item" ref={ref}>
      {leading}
      <ItemText className={mergedCls.itemTextCls} data-slot="select-item-text">
        {children}
      </ItemText>
      {trailing}
