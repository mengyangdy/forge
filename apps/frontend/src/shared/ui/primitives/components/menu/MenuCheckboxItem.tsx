...
const MenuCheckboxItem = forwardRef<ComponentRef<typeof _CheckboxItem>, MenuCheckboxItemProps>(
  (props, ref) => {
    const {
      children,
      className,
    ...
    return (
      <CheckboxItem className={mergedCls} ref={ref} {...rest}>
        <IndicatorComponent className={classNames?.itemIndicator} size={size}>
          {indicatorIcon || <Check />}
        </IndicatorComponent>
        ...
        {shortcut ? (
          <MenuShortcut className={classNames?.shortcut} size={size} value={shortcut} />
        ) : null}
      </CheckboxItem>
    );
  },
