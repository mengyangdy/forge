...
function toTextContent(node: string | number): string {
  return typeof node === "string" ? node : node.toString();
}
...
function skeletonizeChildren(children: ReactNode, options: SkeletonizeOptions): ReactNode {
  ...
  return Children.map(children, (child, index) => {
    // Handle text nodes
    if (isTextNode(child)) {
      const textContent = toTextContent(child);
    ...
    if (isValidElement(child)) {
      ...
      if (isReactComponent(element.type)) {
        ...
        const textContent = hasOnlyTextChildren(props.children)
          ? Children.toArray(props.children).filter(isTextNode).map(toTextContent).join("")
          : "";

        return (
          <span
            className={cn(
              skeletonItemVariants({ animation, type: "interactive" }),
              props.className,
            )}
