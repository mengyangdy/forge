...
type AdminViteConfigValue<T, E extends AdminViteEnv> =
  | T
  | ((context: AdminViteConfigContext<E>) => T);
...
export interface AdminViteUserConfig<E extends AdminViteEnv = AdminViteEnv> {
  /** Forge admin application preset options. */
...
function createServerOptions<E extends AdminViteEnv>(
  ...
  const {
    ...
    open = true,
    port = 9527,
    warmupClientFiles = DEFAULT_WARMUP_CLIENT_FILES,
  } = server || {};
...
function createProxyOptions<E extends AdminViteEnv>(
  ...
  return createAdminViteProxy({
    ...
    serviceConfig:
      resolveConfigValue(proxy?.serviceConfig, context) ??
      createAdminViteServiceConfig(context.env),
  });
}
