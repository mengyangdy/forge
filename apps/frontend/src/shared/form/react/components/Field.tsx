...
import { isEqual, omitUndefined, toArray } from "@forge/shared/utils";
import { getEventValue } from "@forge/shared/utils/web";
...
function Field<Values = any>(props: FieldProps<Values>) {
  ...
  console.log(
    "Field component rendering for:",
    name,
    "fieldContext is null?",
    fieldContext === null,
    "has setFieldValue?",
    fieldContext ? typeof (fieldContext as any).setFieldValue : "none",
  );
  ...
  const controlledProps = omitUndefined({
    ...
    [trigger]: name
      ? (...args: any[]) => {
          ...
          console.log("Field trigger fired for name:", name, "oldValue:", oldValue, "args:", args);
          ...
          if (validateTriggerList.includes(trigger)) {
            void validateField(name, { trigger });
          }
        }
      : undefined,
  });
