...
if (!(globalThis as any).__globalStoreIdCounter) {
  (globalThis as any).__globalStoreIdCounter = 0;
}
...
class FormStore {
  ...
  private use = (mw: Middleware) => {
    ...
    return () => {
      const index = this._middlewares.indexOf(mw);
      if (index === -1) return;
  ...
  private setInitialValues = (values: Store) => {
    ...
    const hasListeners = this._prefixListeners.size > 0 || this._exactListeners.size > 0;
    ...
    const walk = (obj: any, prefix: string[] = []) => {
      Object.keys(obj || {}).forEach((k) => {
        ...
        this.enqueueNotify([key], ChangeTag.Value);
  ...
  private registerField = (entity: FieldEntity) => {
    ...
    const listener = {
      cb: entity.changeValue,
      mask: ChangeTag.Value | ChangeTag.Disabled | ChangeTag.Hidden,
    };
    ...
    if (listeners) {
      ...
      this._exactListeners.set(name, new Set([listener]));
    }
    ...
    return () => {
      ...
      const s = this._exactListeners.get(name);
      if (s) {
        s.delete(listener);
        if (s.size === 0) {
  ...
  private setFieldsValue = (values: Store, validate = false) => {
    ...
    this.transaction(() => {
      ...
      // 4. Batch validate
      if (validate) {
        void this.dispatch({ name: changedKeys, type: "validateFields" });
      }
    });
  };

  private setFieldValue = (name: string, value: StoreValue, validate = false) => {
    console.log("setFieldValue called for name:", name, "value:", value);
    ...
    if (validate) {
      void this.dispatch({ name, type: "validateField" });
    }
  };
  ...
  private _pruneForSubmit(values: Store): Store {
    ...
    const blocked = (k: string) =>
      disabled.some((d) => k.startsWith(d)) || hidden.some((h) => k.startsWith(h));
  ...
  private submit = async (prune: boolean = true) => {
    ...
    if (this._schemaValidator) {
      ...
      if (issues.length > 0) {
        ...
        void this.baseDispatch({ entries, type: "setExternalErrors" });
      }
    }
    ...
    return {
      arrayOp: (name: NamePath) => ({
        insert: (index: number, item: any) => {
          void this.dispatch({ args: { index, item, op: "insert" }, name, type: "arrayOp" });
        },
        move: (from: number, to: number) => {
          void this.dispatch({ args: { from, op: "move", to }, name, type: "arrayOp" });
        },
        remove: (index: number) => {
          void this.dispatch({ args: { index, op: "remove" }, name, type: "arrayOp" });
        },
        replace: (index: number, val: any) => {
          void this.dispatch({ args: { index, item: val, op: "replace" }, name, type: "arrayOp" });
        },
        swap: (index: number, j: number) => {
          void this.dispatch({ args: { from: index, op: "swap", to: j }, name, type: "arrayOp" });
        },
      }),
      ...
      getFieldsValue: (() => {
        const fn = this.getFieldsValue;
        (fn as any).__fnId = (fn as any).__fnId || Math.random();
        return fn;
      })(),
