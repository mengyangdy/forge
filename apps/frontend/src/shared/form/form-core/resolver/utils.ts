...
import { keyOfName } from "@forge/shared/utils";
 ...
 */
export function toEntries<Values = any>(
  issues: StandardSchemaV1NormalizedIssue[],
): [AllPathsKeys<Values>, string[]][] {
...
export function createGenericResolver<Values = any>(
  ...
  return (store) => (next) => async (action) => {
    const dispatch = (value: Action<Values>) => store.dispatch(value);
