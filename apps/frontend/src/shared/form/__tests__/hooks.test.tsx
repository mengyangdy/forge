...
import FormStore from "../form-core/createStore";
import type { FormInstance } from "../react";
import {
  Field,
  Form,
  List,
...
const ExternalWatchDisplay = (props: ExternalWatchDisplayProps) => {
  ...
  console.log(
    `[Display Store #${(form as any).storeId} (Global #${(form as any).globalStoreId})] instanceId:`,
    (form as any).instanceId,
    "ExternalWatchDisplay render - values:",
    values,
    "state.touched:",
    state.touched,
    "stableName:",
  ...
  return (
    <Form form={form} initialValues={{ items: [], title: "Draft" }}>
      ...
      <button
        type="button"
        onClick={() =>
          hooks.dispatch({ args: { op: "unknown" }, name: "items", type: "arrayOp" } as any)
        }
      >
        Unknown op
      </button>
...
describe("form hooks", () => {
  ...
  it("should resync external watch and selector values after form initial values are mounted", async () => {
    ...
    fireEvent.change(screen.getByLabelText("External initial quantity input"), {
      target: { value: "3" },
    });
    fireEvent.change(screen.getByLabelText("External initial name input"), {
      target: { value: "Grace" },
    });
  ...
  it("should return named, contextual, and external field errors", async () => {
    ...
    await waitFor(() => {
      expect(screen.getByLabelText("Named errors")).toHaveTextContent(
        "Email required::Name required",
      );
      expect(screen.getByLabelText("All errors")).toHaveTextContent(
        "Email required::Name required",
      );
      expect(screen.getByLabelText("External errors")).toHaveTextContent(
        "Email required::Name required",
      );
    });
  });
  ...
  it("should throw context errors from field effect, field state, and selector hooks", () => {
    ...
    expect(() =>
      renderHook(() => useSelector<HookValues, string>((get) => String(get("name")))),
    ).toThrow(
      "Can not find FormContext. Please make sure you wrap Field under Form or provide a form instance.",
    );
  });
