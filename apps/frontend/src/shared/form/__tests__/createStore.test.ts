...
import { ChangeTag, addTag } from "../form-core/event";
...
describe("FormStore values and metadata", () => {
  ...
  it("should set nested values, report selected values, and validate changed fields in batch", async () => {
    ...
    hooks.setFieldRules("profile.name", [
      { debounceMs: 0, message: "Name is required", required: true },
    ]);
    hooks.setFieldRules("items.0.name", [
      { debounceMs: 0, message: "Item name is required", required: true },
    ]);
  ...
  it("should recompute computed fields and run field effects", () => {
    ...
    const unregisterComputed = hooks.registerComputed(
      "total",
      ["price", "quantity"],
      (get: (name: string) => any) => {
  ...
  it("should recompute chained computed fields in dependency order", () => {
    ...
    const unregisterTotal = hooks.registerComputed(
      "total",
      ["subtotal"],
      (get: (name: string) => any) => {
  ...
  it("should tolerate stale computed dependency targets", () => {
    ...
    (form as any)
      .getInternalHooks()
      .registerComputed("stable", ["source"], () => form.getFieldValue("stable"));
...
describe("FormStore validation and submit", () => {
  ...
  it("should use debounced validation and cancel stale timers", async () => {
    ...
    registerField(hooks, "email");
    hooks.setFieldRules("email", [
      { debounceMs: 20, message: "Email is required", required: true },
    ]);
