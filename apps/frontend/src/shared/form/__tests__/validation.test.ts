...
import { defaultValidateMessages } from "../form-core/validate";
...
describe("runRulesWithMode", () => {
  ...
  it("should use default messages for built-in type and length checks", async () => {
    const email = await runRulesWithMode(
      "bad-email",
      [{ type: "email" }],
      "parallelAll",
      {},
    ...
    const age = await runRulesWithMode(
      "NaN",
      [{ type: "number" }],
      "parallelAll",
      {},
  ...
  it("should validate numeric, integer, and float rules", async () => {
    ...
    const len = await runRulesWithMode(
      3,
      [{ len: 2, type: "number" }],
      "parallelAll",
      {},
    ...
    const float = await runRulesWithMode(
      2,
      [{ type: "float" }],
      "parallelAll",
      {},
  ...
  it("should accept valid numeric, integer, float, and date values", async () => {
    ...
    const date = await runRulesWithMode(
      "2026-05-07",
      [{ type: "date" }],
      "parallelAll",
      {},
  ...
  it("should validate enum, boolean, hex, regexp, and url rules", async () => {
    ...
    const hexResult = await runRulesWithMode(
      "zzzzzz",
      [{ type: "hex" }],
      "parallelAll",
      {},
    ...
    const regexpResult = await runRulesWithMode(
      "[",
      [{ type: "regexp" }],
      "parallelAll",
      {},
  ...
  it("should accept empty enum rules and valid boolean and regexp values", async () => {
    ...
    const emptyRegexp = await runRulesWithMode(
      "",
      [{ type: "regexp" }],
      "parallelAll",
      {},
    ...
    const regexpObject = await runRulesWithMode(
      {},
      [{ type: "regexp" }],
      "parallelAll",
      {},
  ...
  it("should return an empty result when no rules are provided", async () => {
    await expect(
      runRulesWithMode("value", [], "parallelAll", {}, defaultValidateMessages),
    ).resolves.toEqual({
      errors: [],
      warns: [],
    });
  ...
  it("should use an empty rule fallback in serial mode", async () => {
    await expect(
      runRulesWithMode("value", [undefined as any], "serial", {}, defaultValidateMessages),
    ).resolves.toEqual({
      errors: [],
      warns: [],
    });
