...
import { ChangeTag, addTag, delTag, hasTag, toMask } from "../form-core/event";
import { compose } from "../form-core/middleware";
import { extractSchemaValidator, resolveSchema } from "../form-core/resolver/resolver";
import { createStandardResolver, isStandardSchema } from "../form-core/resolver/standard";
import { createGenericResolver, dispatchIssues, toEntries } from "../form-core/resolver/utils";
...
describe("resolver utilities", () => {
  ...
  it("should intercept field validation and clear a field when it has no matching issue", async () => {
    ...
    await Promise.resolve(middleware({ name: "email", type: "validateField" } as any));
  ...
  it("should validate all requested fields for validateFields actions", async () => {
    ...
    await Promise.resolve(
      middleware({ name: ["email", "profile.name"], type: "validateFields" } as any),
    );
  ...
  it("should validate all fields when validateFields omits names", async () => {
    ...
    await Promise.resolve(middleware({ type: "validateFields" } as any));
  ...
  it("should pass unrelated actions to the next middleware", async () => {
    ...
    await Promise.resolve(middleware(action as any));
...
describe("standard schema resolver", () => {
  ...
  it("should resolve standard schema middleware", async () => {
    ...
    await Promise.resolve(
      middleware({ name: ["user.email", "[object Object]"], type: "validateFields" } as any),
    );
  ...
  it("should resolve successful and root-level standard schema results", async () => {
    ...
    await Promise.resolve(successMiddleware({ type: "validateFields" } as any));
    ...
    await Promise.resolve(issueMiddleware({ type: "validateFields" } as any));
    ...
    await Promise.resolve(noIssueMiddleware({ type: "validateFields" } as any));
