import { createStorage } from "@forge/shared/utils";
import { defaultThemeSettings } from "./config/default";
import { useThemeStore } from "./hooks";
import { getThemeColors, mergeThemeSettings } from "./utils/settings";
import { addThemeVarsToHtml } from "./utils/theme-vars";
 ...
 * 一次调用完成主题初始化
 *
 * 在 app 入口或 SemiProvider 之前调用，自动处理： - 默认配置加载 - localStorage 缓存读取（生产环境） - 版本覆盖检测 - Zustand store 初始化
 *
 * @example
 *   ```ts
...
export function setupTheme(options: SetupThemeOptions = {}) {
  ...
  if (!isProd) {
    ...
    addThemeVarsToHtml(getThemeColors(defaultThemeSettings));
    return;
  }
  ...
  useThemeStore.getState().setSettings(settings);
  addThemeVarsToHtml(getThemeColors(settings));
}
