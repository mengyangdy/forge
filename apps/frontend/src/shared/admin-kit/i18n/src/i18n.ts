...
function configureRuntime(options: LocaleSetupOptions<LangType>) {
  ...
  useLangStore.getState().setFallbackLang(runtimeConfig.fallbackLocale);
  useLangStore.getState().setLocaleOptions(runtimeConfig.localeOptions);
}

function resolveInitialLocale(options: LocaleSetupOptions<LangType>) {
  return (
    options.defaultLocale ?? runtimeConfig.storage?.getLocale() ?? defaultLangConfig.defaultLang
  );
}
    ...
    await i18n.init({
      ...
      missingKeyHandler(lng, _namespace, key) {
        if (options.missingWarn && key.includes(".")) {
          const localeText = Array.isArray(lng) ? lng.join(",") : String(lng);
          console.warn(`[i18next] Not found '${key}' key in '${localeText}' locale messages.`);
        }
      },
      react: {
        useSuspense: false,
...
export async function loadLocaleMessages(lang: LangType) {
  ...
  const messages = loadAdminLocaleMessages(locale);
  ...
  useLangStore.getState().setLocale(locale);
