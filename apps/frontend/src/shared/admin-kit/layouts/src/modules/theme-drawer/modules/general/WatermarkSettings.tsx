...
import { Input as AInput, Select as ASelect, Switch as ASwitch } from "@douyinfe/semi-ui";
...
const WatermarkSettings = () => {
  ...
  const { setSettings, setWatermarkEnableTime, setWatermarkEnableUserName, watermark } =
    useSettingsTheme();
  ...
  const handleTimeFormatChange = (timeFormat: string | string[] | undefined) => {
      ...
      <AnimatePresence initial={false} mode="popLayout">
        ...
        <AnimatedItem key="timeFormat" visible={watermark.visible && watermark.enableTime}>
          <SettingItem label={t("theme.general.watermark.timeFormat")}>
            <ASelect
              className="w-210px"
              optionList={watermarkTimeFormatOptions}
              size="small"
