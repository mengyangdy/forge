...
class MenuGenerator {
  ...
  private generateStaticMenus(options: GenerateMenuOptions): GenerateMenuResult {
    ...
    const menuCategoryLayouts = getConfiguredCategoryKeys().map(
      (layout) => menuCategories[layout].layout,
    );
    ...
    layoutRoutes.forEach((layoutRoute) => {
      ...
      const menus = this.generateStaticLayoutMenus(
        layoutRoute,
  ...
  private generateStaticChildMenus(options: StaticChildMenusOptions) {
    ...
    if (route.children && Object.keys(route.children).length > 0) {
      Object.values(route.children).forEach((childRoute) => {
        const typedChildRoute = childRoute as AnyRoute;
        const childPath = normalizePath(typedChildRoute.fullPath);

        // Index routes under a layout share the layout fullPath after normalization
        // (e.g. /system and /system/ → /system). Skip them so menu item keys stay unique.
  ...
  private transformBackendRouteToMenu(
    ...
    const children = route.children
      ?.map((child) =>
        this.transformBackendRouteToMenu(
          child,
          quickReferenceMenuMap,
          userInfo,
          [...parentKeys, path],
