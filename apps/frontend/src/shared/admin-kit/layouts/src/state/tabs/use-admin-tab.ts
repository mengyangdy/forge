...
interface TabStore {
  ...
  tabs: App.Global.Tab[];
  activeTabId: string;
  setTabState: (update: Partial<Omit<TabStore, "setTabState">>) => void;
}
...
export const useTabStore = create<TabStore>((set) => ({
  ...
  tabs: [],
  activeTabId: "",
  setTabState: (update) => set((state) => ({ ...state, ...update })),
}));
...
export const useAdminTab = () => {
  ...
  const setTabState = useTabStore((state) => state.setTabState);
  ...
  function addTab(fullPath: string, routePath: Router.RoutePath, active = true) {
    ...
    const oldTabs = useTabStore.getState().tabs;
    ...
    if (!isHomeTab && existingTab) {
      setTabState({
        tabs: oldTabs.map((item) => (item.id === tab.id ? { ...item, ...tab } : item)),
      });
    }
  ...
  async function removeTab(tabId: string) {
    ...
    const nextTab =
      tabState.tabs[removeTabIndex + 1] || tabState.tabs[removeTabIndex - 1] || tabState.homeTab;
