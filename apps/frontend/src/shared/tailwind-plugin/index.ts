...
import { presetForgeUI } from "./presets";
import themes from "./theme.json";
import { forgeUITheme } from "./theme-presets";
import type {
  ForgeUIPluginOptions,
  ThemeColorKey,
  ThemeConfig,
  ThemeConfigColor,
  ThemeOptions,
...
type ForgeUIPlugin = {
  (options?: ForgeUIPluginOptions): { config?: Partial<Config>; handler: PluginCreator };
...
/**
 * The UnoCSS preset for Forge UI.
 *
 ...
 */
export const forgeUIPlugin: ForgeUIPlugin = plugin.withOptions(
  (options: ForgeUIPluginOptions = {}) =>
    (api) => {
      api.addUtilities(presetForgeUI());

      api.addBase(forgeUITheme(options));
    },
  (options: ForgeUIPluginOptions = {}) => {
...
export { generateCSSVars, presetForgeUI, forgeUITheme };

export type { ThemeColorKey, ThemeConfig, ThemeConfigColor, ThemeOptions };

export default forgeUIPlugin;
