...
function buildMainColorPalette(name: string) {
  return {
    50: `var(--${name}-50)`,
    100: `var(--${name}-100)`,
    200: `var(--${name}-200)`,
...
function createColorsPalette(name: string) {
  const colors = buildMainColorPalette(name);
  return {
    ...colors,
    DEFAULT: `var(--${name})`,
 ...
 * - 等等
 *
 * 允许使用类似 `text-primary-50`、`bg-success-100` 等类名
 */
