...
import { getColorPalette, getPaletteColorByNumber } from "../../../src/color/palette/index.js";
...
describe("getColorPalette", () => {
  ...
  it("三种算法应返回不同结果", () => {
    const hsv = getColorPalette("#1890ff", "hsv");
    const recommended = getColorPalette("#1890ff", "recommended");
    const oklch = getColorPalette("#1890ff", "oklch");

    expect(hasDifference(hsv, recommended)).toBe(true);
    expect(hasDifference(hsv, oklch)).toBe(true);
