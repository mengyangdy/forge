...
import {
  ...
  triggerDownload,
  urlToBase64,
} from "../../../src/utils/web/download.js";
    ...
    set(_: string) {
      setTimeout(() => {
        if (overrides.failOnLoad) {
          emitMockImageEvent(this, "error");
        } else {
          emitMockImageEvent(this, "load");
        }
      }, 0);
    },
  });
...
describe("downloadFileFromBlob", () => {
  ...
  it("非 Blob 应抛出 TypeError", () => {
    expect(() => downloadFileFromBlob({ source: "not a blob" as any })).toThrow(
      "Invalid Blob data.",
    );
  });
});
...
describe("downloadFileFromImageUrl", () => {
  it("无效 source 应抛错", async () => {
    ...
    await expect(downloadFileFromImageUrl({ source: null as any })).rejects.toThrow(
      "Invalid image URL.",
    );
  });
    ...
    vi.stubGlobal(
      ...
      class extends originalURL {
        ...
        static override revokeObjectURL = vi.fn(
          (...args: Parameters<typeof originalURL.revokeObjectURL>) =>
            originalURL.revokeObjectURL(...args),
        );
      },
    );
    ...
    vi.stubGlobal(
      ...
      class extends originalURL {
        ...
        static override createObjectURL = vi.fn(
          (...args: Parameters<typeof originalURL.createObjectURL>) =>
            originalURL.createObjectURL(...args),
        );
        static override revokeObjectURL = vi.fn(
          (...args: Parameters<typeof originalURL.revokeObjectURL>) =>
            originalURL.revokeObjectURL(...args),
