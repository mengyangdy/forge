...
import { getColorName } from "../shared/index.js";
import type {
  ColorPalette,
  ColorPaletteFamily,
  ColorPaletteMatch,
...
export type ColorPaletteWithOklch = ColorPalette & {
...
export function generateOklchPaletteAdvanced(
  color: string,
  ...
  const referenceCompensation = chromaCompensation
    ? getHueAwareChromaCompensation(closestStep.targetL, safeHue)
    : 1;
  const baseChroma = safeChroma / Math.max(referenceCompensation, 0.1);
 ...
 */
export function generateOklchPaletteEx(
  color: string,
  forceStep?: ColorPaletteNumber,
